...
# Apni actual relationship start date daalo
# Format: Year, Month, Day, Hour, Minute
RELATIONSHIP_START = datetime(2023, 1, 15, 18, 30)

# Photos /static/ folder mein daalo in naamon se
PHOTO_CONFIG = [
...
# =====================================================================
#  MAIN HTML PAGE — LUXURY DARK ROMANTIC DESIGN
# =====================================================================
...
<head>
  ...
  <style>
    ...
    *, *::before, *::after { margin:0; padding:0; box-sizing:border-box; }
    ...
    /* ══════════════════════════════════════════════════════════════════
       BACKGROUND CANVAS — animated star-dust
    ══════════════════════════════════════════════════════════════════ */
    #bgCanvas {
    ...
    @keyframes petalFall {
      ...
      8%   { opacity: 0.7; }
      90%  { opacity: 0.4; }
      100% { transform: translateY(108vh) translateX(30px) rotate(400deg); opacity:0; }
    }
    ...
    .hero-ornament {
      ...
      animation: fadeSlideDown 1.4s cubic-bezier(.16,1,.3,1) both;
    }
    ...
    .hero-title {
      ...
      animation: fadeSlideDown 1.6s cubic-bezier(.16,1,.3,1) 0.15s both;
    }
    .hero-title span {
      display: block;
    ...
    .hero-sub {
      ...
      animation: fadeSlideDown 1.8s cubic-bezier(.16,1,.3,1) 0.3s both;
    }
    ...
    .hero-date {
      ...
      text-transform: uppercase;
      animation: fadeSlideDown 2s cubic-bezier(.16,1,.3,1) 0.45s both;
    }
    ...
    .scroll-hint {
      ...
      align-items: center;
      gap: 6px;
      animation: fadeSlideDown 2.2s 1s both;
      cursor: default;
    }
    .scroll-hint span {
      font-family: var(--display);
    ...
    .polaroid:hover {
      ...
      box-shadow: 0 25px 60px rgba(0,0,0,0.7), 0 0 30px rgba(201,168,76,0.15);
    ...
    /* ══════════════════════════════════════════════════════════════════
       ⑤ REASONS I LOVE YOU — horizontal scroll cards
    ══════════════════════════════════════════════════════════════════ */
    ...
    .song-item:hover .song-play { background: var(--gold); color: var(--black); transform: scale(1.1); }
    .song-duration { font-size:0.75rem; color: var(--text2); flex-shrink:0; font-family: var(--display); letter-spacing:0.05em; }
    .song-player-embed { border-radius:14px; overflow:hidden; margin: 0.6rem 0 0.3rem; }
    ...
    .flip-front {
      background: linear-gradient(145deg, var(--velvet2), var(--velvet3));
      border: 1px solid rgba(201,168,76,0.25);
      box-shadow: 0 6px 30px rgba(0,0,0,0.4);
    }
    .flip-back {
      transform: rotateY(180deg);
      background: linear-gradient(145deg, #1e0f30, #2a1045);
    ...
    .gift-body {
      ...
      height: 115px;
      background: linear-gradient(145deg, #7a1530, #b5294e);
      border-radius: 0 0 18px 18px;
      box-shadow: 0 12px 40px rgba(181,41,78,0.4);
    }
    .gift-ribbon-v {
      position:absolute; bottom:0; left:50%; transform:translateX(-50%);
      width:28px; height:115px;
    ...
    .gift-lid {
      ...
      height:50px;
      background: linear-gradient(145deg, #9a1c3a, #c9335c);
      border-radius: 10px;
    ...
    .wish-star {
      ...
      animation: starTwinkle var(--d, 2s) ease-in-out infinite alternate;
    }
    ...
    .modal-inner {
      background: var(--velvet2);
      border: 1px solid rgba(201,168,76,0.25);
      border-radius: 28px;
      max-width: 440px; width:100%;
      padding: 2.5rem 2rem;
      text-align:center;
      box-shadow: 0 40px 80px rgba(0,0,0,0.7), 0 0 40px rgba(201,168,76,0.08);
    ...
    #musicBar {
      ...
      box-shadow: 0 8px 40px rgba(0,0,0,0.5), 0 0 20px rgba(201,168,76,0.06);
      transition: transform 0.2s, box-shadow 0.2s;
    }
    #musicBar:hover { transform: scale(1.03); box-shadow: 0 12px 50px rgba(0,0,0,0.6), 0 0 30px rgba(201,168,76,0.12); }
    #musicBtn {
    ...
    #musicStatus { font-size: 0.6rem; color: var(--text2); font-family: var(--serif); font-style:italic; }
...
<div id="musicBar">
  ...
  <div>
    <div id="musicLabel">Romantic Beats</div>
    <div id="musicStatus">Tap to play</div>
  </div>
</div>
<iframe id="ytFrame" style="display:none;position:absolute;width:1px;height:1px;opacity:0;" allow="autoplay"></iframe>
...
<section id="songs">
  <div class="section-wrap">
    <div class="reveal">
      <p class="section-label">A Playlist</p>
      <h2 class="section-title">Tumhare Liye <em>Kuch Gaane</em></h2>
      <div class="gold-line"></div>
    </div>
...
const SONGS = [
  { title:"Tum Hi Ho",           artist:"Arijit Singh — Aashiqui 2",           yt:"IJq0yyWBNmM", dur:"4:22" },
  { title:"Ae Dil Hai Mushkil",  artist:"Arijit Singh",                         yt:"6FURuLYrR_Q", dur:"5:01" },
  { title:"Tera Ban Jaunga",     artist:"Tulsi Kumar",                           yt:"gVbFNMHBhbI", dur:"3:58" },
  { title:"Pehla Nasha",        artist:"Jo Jeeta Wohi Sikandar",               yt:"q5xROmcXFV8", dur:"5:14" },
  { title:"Kuch Toh Hua Hai",   artist:"Sonu Nigam — Kal Ho Na Ho",            yt:"1FG0BkZJ5Z4", dur:"4:45" },
  { title:"Raabta",              artist:"Arijit Singh",                          yt:"JMQMRmVlCRA", dur:"4:11" },
  { title:"Hawayein",            artist:"Arijit Singh — Jab Harry Met Sejal",   yt:"cGGNSVsAP9g", dur:"4:39" },
  { title:"Main Teri Ho Gayi",  artist:"Millind Gaba",                         yt:"lkVq5Gc1Uf4", dur:"3:35" },
];
...
let twIdx = 0;
function typeNext() {
  if (twIdx <= LOVE_LETTER.length) {
    TW_EL.textContent = LOVE_LETTER.slice(0, twIdx);
    twIdx++;
    setTimeout(typeNext, 38);
  } else {
    TW_EL.classList.add('done');
  }
}
// Start typewriter when letter section enters view
const letterObs = new IntersectionObserver(entries => {
  if (entries[0].isIntersecting && twIdx === 0) { twIdx=1; typeNext(); }
}, { threshold: 0.2 });
...
PHOTOS.forEach((p, i) => {
  ...
  card.innerHTML = `
    <img src="/static/${p.img}" alt="${p.title}"
         onerror="this.src='https://placehold.co/400x300/1a0e28/c9a84c?text=♥'">
    <div class="polaroid-caption">
      <strong>${p.title}</strong>
      <p>${p.desc}</p>
    </div>`;
  card.addEventListener('click', () => openModal(`
    <img class="modal-img" src="/static/${p.img}" onerror="this.src='https://placehold.co/440x280/1a0e28/c9a84c?text=♥'">
...
REASONS.forEach((r, i) => {
  ...
  card.innerHTML = `
    ...
    <span class="reason-num">Reason ${String(i+1).padStart(2,'0')}</span>
...
SONGS.forEach((s, i) => {
  ...
  row.innerHTML = `
    ...
    <div class="song-play" id="sp${i}">▶</div>`;
  row.addEventListener('click', () => {
    document.querySelectorAll('.song-play').forEach(b => b.textContent = '▶');
    const old = document.getElementById('songEmbed');
    if (old) old.remove();
    const embed = document.createElement('div');
    embed.id = 'songEmbed';
    embed.className = 'song-player-embed';
    embed.innerHTML = `<iframe width="100%" height="80" src="https://www.youtube.com/embed/${s.yt}?autoplay=1" frameborder="0" allow="autoplay;encrypted-media" allowfullscreen></iframe>`;
    row.after(embed);
  ...
  wrap.innerHTML = `
    <div class="flip-inner">
      <div class="flip-face flip-front">
        <div class="flip-icon">${s.icon}</div>
        <div class="flip-label">${s.label}</div>
        <div class="flip-hint">tap to reveal</div>
      </div>
      <div class="flip-face flip-back">
        <p class="flip-msg">${s.msg}</p>
      </div>
    </div>`;
  wrap.addEventListener('click', () => {
    wrap.classList.toggle('flipped');
    if (wrap.classList.contains('flipped')) launchFireworks();
  });
  flipGrid.appendChild(wrap);
...
giftWrap.addEventListener('click', () => {
  ...
  setTimeout(() => openModal(`
    ...
    <div class="modal-title">Meri Jaan — Ye Tohfa Tumhare Liye</div>
    <div class="modal-desc" style="margin-top:1rem">
      <strong style="color:var(--gold2);display:block;margin-bottom:0.6rem">"Good for 1000 Hugs + Unlimited Pyaar"</strong>
      Har baar gale lagana mera haq hai — aur main hamesha tumhe yaad dilata rahunga ke main tumse kitna pyaar karta hoon.<br><br>
      <span style="color:var(--rose3)">✦ Redeem Code: MUSTAFA_LOVES_BEGAMJI_FOREVER ✦</span>
    </div>
    <button class="modal-close" onclick="closeModal()">Accept With Love</button>
  `), 500);
});
...
(function () {
  ...
  let lit      = true;
  let blown    = 0; // how many candles blown
  const TOTAL  = 5;
  ...
  function drawCake() {
    ...
    // Plate
    ctx.beginPath();
    ctx.ellipse(150, 295, 130, 18, 0, 0, Math.PI*2);
    ...
    // Bottom tier
    const g1 = ctx.createLinearGradient(45, 195, 255, 275);
    g1.addColorStop(0, '#7a1530'); g1.addColorStop(1, '#b5294e');
    ctx.beginPath();
    ctx.moveTo(50, 268); ctx.lineTo(250, 268);
    ctx.bezierCurveTo(250, 290, 50, 290, 50, 268);
    ctx.lineTo(50, 210);
    ctx.bezierCurveTo(50, 192, 250, 192, 250, 210);
    ctx.lineTo(250, 268);
    ctx.fillStyle = g1; ctx.fill();

    // Gold stripe bottom
    ctx.fillStyle = 'rgba(201,168,76,0.35)';
    ctx.fillRect(50, 258, 200, 8);

    // Middle tier
    const g2 = ctx.createLinearGradient(65, 130, 235, 195);
    g2.addColorStop(0, '#6d1028'); g2.addColorStop(1, '#a02040');
    ctx.beginPath();
    ctx.moveTo(65, 192); ctx.lineTo(235, 192);
    ctx.bezierCurveTo(235, 210, 65, 210, 65, 192);
    ctx.lineTo(65, 138);
    ctx.bezierCurveTo(65, 120, 235, 120, 235, 138);
    ctx.lineTo(235, 192);
    ctx.fillStyle = g2; ctx.fill();

    // Gold stripe mid
    ctx.fillStyle = 'rgba(201,168,76,0.35)';
    ctx.fillRect(65, 183, 170, 7);

    // Top tier
    const g3 = ctx.createLinearGradient(85, 72, 215, 125);
    g3.addColorStop(0, '#5c0d1f'); g3.addColorStop(1, '#8b1a38');
    ctx.beginPath();
    ctx.moveTo(85, 128); ctx.lineTo(215, 128);
    ctx.bezierCurveTo(215, 145, 85, 145, 85, 128);
    ctx.lineTo(85, 82);
    ctx.bezierCurveTo(85, 65, 215, 65, 215, 82);
    ctx.lineTo(215, 128);
    ctx.fillStyle = g3; ctx.fill();

    // Gold stripe top
    ctx.fillStyle = 'rgba(201,168,76,0.35)';
    ctx.fillRect(85, 119, 130, 7);

    // Drip decorations
    [[50,204],[85,204],[130,204],[175,204],[220,204]].forEach(([x,y]) => {
      ctx.beginPath(); ctx.arc(x,y,7,0,Math.PI*2);
      ctx.fillStyle='rgba(201,168,76,0.25)'; ctx.fill();
    });
    [[65,143],[100,143],[150,143],[200,143]].forEach(([x,y]) => {
    ...
    ctx.fillStyle = 'rgba(247,238,212,0.75)';
    ctx.textAlign = 'center';
    ctx.fillText('Happy Birthday', 150, 100);
    ctx.font = 'italic 9px "Cormorant Garamond", serif';
    ctx.fillStyle = 'rgba(201,168,76,0.8)';
    ctx.fillText('Begam Ji ♥', 150, 114);
    ctx.textAlign = 'left';
    ...
    CX.forEach((x, i) => {
      const isLit = i >= blown;
      // body
      const cg = ctx.createLinearGradient(x-5,42,x+5,65);
      const cols = ['#c9a84c','#b5294e','#c9a84c','#b5294e','#c9a84c'];
      cg.addColorStop(0, cols[i]); cg.addColorStop(1, '#fff');
      ctx.fillStyle = cg;
      ctx.beginPath();
      ctx.roundRect ? ctx.roundRect(x-5,42,10,24,3) : (()=>{ctx.rect(x-5,42,10,24);})();
      ctx.fill();
      ctx.strokeStyle='rgba(0,0,0,0.2)'; ctx.lineWidth=0.5; ctx.stroke();
      // wick
      ctx.beginPath(); ctx.moveTo(x,42); ctx.lineTo(x,36);
      ...
      if (isLit) {
        ...
        fg.addColorStop(0.4,'#f5c518');
        fg.addColorStop(1,'rgba(245,70,0,0)');
        ctx.beginPath();
        ctx.moveTo(x+flick,22);
        ctx.bezierCurveTo(x+5+flick,28,x+4+flick,35,x+flick,37);
        ctx.bezierCurveTo(x-4+flick,35,x-5+flick,28,x+flick,22);
...
WISHES.forEach((msg, i) => {
  ...
  star.textContent = '✦';
  star.style.setProperty('--d', (1.2 + i*0.4) + 's');
  star.style.color = i%2===0 ? 'var(--gold)' : 'var(--rose3)';
  star.addEventListener('click', () => {
    document.querySelectorAll('.wish-star').forEach(s => s.classList.remove('granted'));
    star.classList.add('granted');
    const box = document.getElementById('wishGranted');
    box.style.display = 'block';
    document.getElementById('wishMsg').textContent = msg;
    launchFireworks();
  });
...
function launchFireworks() {
  ...
  const ctx = c.getContext('2d');
  const COLS = ['#c9a84c','#e8d08a','#b5294e','#f0a0bb','#ffffff','#d4567a'];
  let particles = [];
  ...
  function burst(ox, oy) {
    for (let i=0; i<90; i++) {
      ...
      particles.push({
        ...
        a:1, decay: 0.012+Math.random()*0.015
      });
    }
  }

  burst(c.width*0.35, c.height*0.35);
  setTimeout(() => burst(c.width*0.65, c.height*0.4),  250);
  setTimeout(() => burst(c.width*0.5,  c.height*0.25), 500);
...
musicBtn.addEventListener('click', () => {
  musicOn = !musicOn;
  if (musicOn) {
    ytFrame.src = 'https://www.youtube.com/embed/IJq0yyWBNmM?autoplay=1&loop=1&playlist=IJq0yyWBNmM';
    ytFrame.style.display = 'block';
    musicStatus.textContent = 'Now playing ♪';
    musicBtn.textContent = '⏸';
    musicBtn.style.background = 'linear-gradient(135deg, #c9a84c, #e8d08a)';
    musicBtn.style.color = '#06030a';
  } else {
    ytFrame.src = '';
...
document.getElementById('heroDate').textContent =
  months[now.getMonth()] + ' ' + now.getDate() + ', ' + now.getFullYear() + '  ·  From Mustafa, with endless love';
...
def get_start_params():
    return {
        "year":   RELATIONSHIP_START.year,
        "month":  RELATIONSHIP_START.month - 1,
        "day":    RELATIONSHIP_START.day,
        "hour":   RELATIONSHIP_START.hour,
        "minute": RELATIONSHIP_START.minute,
    }
...
<head>
  ...
  <style>
    *{margin:0;padding:0;box-sizing:border-box;}
    body {
      min-height:100vh;
      background: radial-gradient(ellipse 120% 100% at 50% 0%, #1a0530 0%, #06030a 60%);
      display:flex; align-items:center; justify-content:center;
      font-family:'Cormorant Garamond',serif;
    ...
    .petal  { position:absolute; font-size:1rem; animation:pfall linear forwards; opacity:0.5; }
    @keyframes pfall {
    ...
    .card {
      ...
      text-align:center;
      box-shadow: 0 0 80px rgba(201,168,76,0.07), 0 20px 60px rgba(0,0,0,0.7);
    ...
    .ornament {
      font-family:'Cinzel',serif;
      font-size:0.6rem; letter-spacing:0.5em; color:#c9a84c;
      text-transform:uppercase; opacity:0.7; margin-bottom:1.2rem;
    }
    h1 {
      font-family:'Cinzel',serif;
      font-size:1.9rem; font-weight:400;
      color:#f5edd8; letter-spacing:0.06em;
    ...
    input[type=password] {
      ...
      border:1px solid rgba(201,168,76,0.2);
      border-radius:50px; color:#f5edd8;
      font-family:'Cormorant Garamond',serif;
    ...
    button[type=submit]:hover {
      ...
      box-shadow:0 10px 35px rgba(181,41,78,0.4);
    }
...
<script>
  ...
  window.addEventListener('resize',resize);resize();
  for(let i=0;i<180;i++)stars.push({x:Math.random()*W,y:Math.random()*H,r:Math.random()*1+0.2,a:Math.random(),s:(Math.random()-0.5)*0.004});
  function draw(){ctx.clearRect(0,0,W,H);stars.forEach(s=>{s.a+=s.s;if(s.a<0||s.a>1)s.s*=-1;ctx.beginPath();ctx.arc(s.x,s.y,s.r,0,Math.PI*2);ctx.fillStyle=`rgba(201,168,76,${s.a*0.7})`;ctx.fill();});requestAnimationFrame(draw);}
  draw();
  // petals
  const PE=['🌹','✦','·','❧'];
  setInterval(()=>{
    const e=document.createElement('div');e.className='petal';
    e.textContent=PE[Math.floor(Math.random()*PE.length)];
    e.style.left=Math.random()*100+'vw';
    e.style.animationDuration=(5+Math.random()*8)+'s';
    e.style.fontSize=(0.6+Math.random()*0.7)+'rem';
    e.style.color=Math.random()>.5?'#c9a84c':'#b5294e';
    document.getElementById('petalDiv').appendChild(e);
...
<head>
  ...
  <style>
    *{margin:0;padding:0;box-sizing:border-box;}
    body{min-height:100vh;background:#06030a;display:flex;align-items:center;justify-content:center;font-family:'Cormorant Garamond',serif;}
    .box{text-align:center;padding:4rem 3rem;position:relative;}
    .heart-ring{width:120px;height:120px;border-radius:50%;border:1px solid rgba(201,168,76,0.3);margin:0 auto 2.5rem;display:flex;align-items:center;justify-content:center;animation:ringPulse 1.8s ease-in-out infinite;}
    @keyframes ringPulse{0%,100%{box-shadow:0 0 0 0 rgba(201,168,76,0.3);}50%{box-shadow:0 0 0 20px rgba(201,168,76,0);}}
    .heart-icon{font-size:2.8rem;animation:heartbeat 1.2s ease-in-out infinite;}
    @keyframes heartbeat{0%,100%{transform:scale(1);}50%{transform:scale(1.18);}}
    h2{font-family:'Cinzel',serif;font-size:1.1rem;letter-spacing:0.2em;color:#c9a84c;text-transform:uppercase;margin-bottom:0.8rem;}
    p{font-style:italic;color:#a89580;font-size:1rem;}
...
<body>
<div class="box">
  <div class="heart-ring"><div class="heart-icon">❤️</div></div>
  <h2>Thoda Sa Intezaar</h2>
  <p>Aapke liye kuch khaas tayyar ho raha hai<span class="dots"></span></p>
</div>
</body>
</html>"""
...
<head>
  ...
  <style>
    *{margin:0;padding:0;box-sizing:border-box;}
    body{min-height:100vh;background:#06030a;display:flex;align-items:center;justify-content:center;font-family:'Cormorant Garamond',serif;}
    .box{background:rgba(10,5,18,0.85);border:1px solid rgba(181,41,78,0.25);border-radius:24px;padding:3.5rem 2.5rem;max-width:400px;width:90%;text-align:center;box-shadow:0 20px 60px rgba(0,0,0,0.6);}
    .icon{font-size:3rem;margin-bottom:1.5rem;display:block;}
    h2{font-family:'Cinzel',serif;font-size:1rem;letter-spacing:0.25em;color:#b5294e;text-transform:uppercase;margin-bottom:1rem;}
    p{font-style:italic;color:#a89580;font-size:1rem;line-height:1.7;margin-bottom:1.5rem;}
    a{display:inline-block;padding:0.75rem 2.5rem;border:1px solid rgba(201,168,76,0.3);border-radius:50px;color:#c9a84c;font-family:'Cinzel',serif;font-size:0.65rem;letter-spacing:0.25em;text-transform:uppercase;text-decoration:none;transition:background 0.25s;}
    a:hover{background:rgba(201,168,76,0.08);}
  </style>
</head>
<body>
<div class="box">
  ...
  <h2>Galat Code</h2>
  <p>Apne sohar Mustafa se password le aaiye — woh zaroor batayenge 🥺</p>
  <a href="/">Wapas Jaiye →</a>
</div>
</body>
</html>"""
